    ...
    def chat_response(self, message: str, history: List[Dict]) -> Tuple[List[Dict], str]:
        ...
        try:
            ...
            if self.current_data is None:
                ...
                # Add timestamped messages to history
                user_message = {
                    "role": "user", 
            ...
            # Add timestamped error messages
            user_message = {
                "role": "user", 
                "content": self._add_timestamp_to_message(message, error_timestamp)
            }
            assistant_message = {
                "role": "assistant", 
                "content": self._add_timestamp_to_message(error_response, error_timestamp)
            }
...
🆔 **Session ID**: {self.session_id if hasattr(self, 'session_id') else 'loading...'}<span class="dots">...</span><span class="more-text">
...
setTimeout(() => {{
        ...
        container.parentNode.appendChild(toggleBtn);
            ...
            def handle_file_upload(file, chatbot):
                ...
                if analysis_message:
                    ...
                    user_msg = {"role": "user", "content": self._add_timestamp_to_message("CSV File Loaded", current_timestamp)}
                    assistant_msg = {"role": "assistant", "content": self._add_timestamp_to_message(bot_message, current_timestamp)}
                    updated_chatbot = updated_chatbot + [user_msg, assistant_msg]
                ...
                try:
                    ...
                    news_timestamp = datetime.now().strftime("%H:%M:%S")
                    ...
                    if news_results and isinstance(news_results, dict) and news_results.get('search_queries'):
                        ...
                        user_msg = {"role": "user", "content": self._add_timestamp_to_message("Business Context Analysis", news_timestamp)}
                        assistant_msg = {"role": "assistant", "content": self._add_timestamp_to_message(news_content, news_timestamp)}
                        updated_chatbot = updated_chatbot + [user_msg, assistant_msg]
                        ...
                        user_msg = {"role": "user", "content": self._add_timestamp_to_message("Business Context Analysis", news_timestamp)}
                        assistant_msg = {"role": "assistant", "content": self._add_timestamp_to_message(fallback_news_content, news_timestamp)}
                        updated_chatbot = updated_chatbot + [user_msg, assistant_msg]
                    ...
                    user_msg = {"role": "user", "content": self._add_timestamp_to_message("Business Context Analysis", error_timestamp)}
                    assistant_msg = {"role": "assistant", "content": self._add_timestamp_to_message(error_content, error_timestamp)}
                    updated_chatbot = updated_chatbot + [user_msg, assistant_msg]
    ...
    def _classify_user_intent(self, query: str) -> Optional[str]:
        """Use LLM to classify user query intent and route to appropriate analysis"""
        try:
...
"""
            ...
            try:
                ...
                assumptions = [
                    ...
                    f"Showing {directionText.lower()} {n} performers",
