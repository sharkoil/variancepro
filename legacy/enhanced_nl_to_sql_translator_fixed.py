...
class EnhancedNLToSQLTranslator:
    ...
    def _extract_query_elements(self, query: str, context: QueryContext):
        ...
        self._apply_smart_fixes(query, context)
    ...
    def _apply_smart_fixes(self, query: str, context: QueryContext):
        """Apply smart fixes for common query patterns"""
        
        # Fix 1: Check for "actual sales is less than budget sales" pattern
        if "actual sales" in query and "less than" in query and "budget sales" in query:
            # Replace with direct column comparison
            for i, filter_dict in enumerate(context.filters):
                if filter_dict.get('column') == 'budget_sales' and isinstance(filter_dict.get('value'), str) and 'less than budget sales' in filter_dict.get('value', ''):
                    # Replace with proper column comparison
                    context.filters[i] = {
                        'column': 'actual_sales',
                        'operator': '<',
        ...
        # Fix 2: Check for "discount percentage" queries
        if "discount" in query and "%" in query:
            # Look for percentage value in filters
            for i, filter_dict in enumerate(context.filters):
                if filter_dict.get('column') != 'discount_pct':
                    if 'discount' in query:
                        # Replace with proper discount column
                        context.filters[i]['column'] = 'discount_pct'
                        
        # Fix 3: Check for "sales variance is negative" pattern
        if "variance" in query and ("negative" in query or "below zero" in query):
            # Look for "negative" value in filters
            for i, filter_dict in enumerate(context.filters):
                if isinstance(filter_dict.get('value'), str) and 'negative' in filter_dict.get('value', ''):
                    # Replace with proper comparison to zero
                    context.filters[i] = {
        ...
        # Fix 4: Check for "highest customer satisfaction" pattern
        if "highest" in query and "satisfaction" in query:
            # Add ordering by customer satisfaction
            satisfaction_col = self._resolve_column_name("customer satisfaction")
            if satisfaction_col:
                context.order_by = {
                    'column': satisfaction_col,
                    'direction': 'DESC'
                }
